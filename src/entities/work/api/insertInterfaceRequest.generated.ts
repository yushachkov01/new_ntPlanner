...
export type InsertInterfaceRequestMutationVariables = Types.Exact<{
  dev: Types.Scalars['uuid']['input'];
  type: Types.Scalars['String']['input'];
}>;

export type InsertInterfaceRequestMutation = {
  __typename?: 'mutation_root';
  insert_public7_query_one?: { __typename?: 'public7_query'; id: any } | null;
};

export const InsertInterfaceRequestDocument = gql`
  mutation InsertInterfaceRequest($dev: uuid!, $type: String!) {
    insert_public7_query_one(object: { device: $dev, request_type: $type }) {
      id
    }
  }
`;
...
export function getSdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
  return {
    InsertInterfaceRequest(
      variables: InsertInterfaceRequestMutationVariables,
      requestHeaders?: GraphQLClientRequestHeaders,
      signal?: RequestInit['signal'],
    ): Promise<InsertInterfaceRequestMutation> {
      return withWrapper(
        (wrappedRequestHeaders) =>
          client.request<InsertInterfaceRequestMutation>({
            document: InsertInterfaceRequestDocument,
            variables,
