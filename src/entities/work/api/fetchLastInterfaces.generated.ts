...
export type FetchLastInterfacesQueryVariables = Types.Exact<{
  dev: Types.Scalars['uuid']['input'];
  type: Types.Scalars['String']['input'];
}>;

export type FetchLastInterfacesQuery = {
  __typename?: 'query_root';
  public7_query: Array<{
    __typename?: 'public7_query';
    status: string;
...
export const FetchLastInterfacesDocument = gql`
  query FetchLastInterfaces($dev: uuid!, $type: String!) {
    public7_query(
      where: { device: { _eq: $dev }, request_type: { _eq: $type } }
      order_by: { received_at: desc_nulls_last }
      limit: 1
    ) {
      status
      received_at
...
export function getSdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
  return {
    FetchLastInterfaces(
      variables: FetchLastInterfacesQueryVariables,
      requestHeaders?: GraphQLClientRequestHeaders,
      signal?: RequestInit['signal'],
    ): Promise<FetchLastInterfacesQuery> {
      return withWrapper(
        (wrappedRequestHeaders) =>
          client.request<FetchLastInterfacesQuery>({
            document: FetchLastInterfacesDocument,
            variables,
