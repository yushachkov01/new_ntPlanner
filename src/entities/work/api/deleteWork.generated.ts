...
export type DeleteWorkMutationVariables = Types.Exact<{
  where: Types.Works_Bool_Exp;
}>;


export type DeleteWorkMutation = { __typename?: 'mutation_root', delete_works?: { __typename?: 'works_mutation_response', affected_rows: number } | null };


export const DeleteWorkDocument = gql`
    mutation DeleteWork($where: works_bool_exp!) {
  delete_works(where: $where) {
    affected_rows
  }
}
    `;
...
export function getSdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
  return {
    DeleteWork(variables: DeleteWorkMutationVariables, requestHeaders?: GraphQLClientRequestHeaders, signal?: RequestInit['signal']): Promise<DeleteWorkMutation> {
      return withWrapper((wrappedRequestHeaders) => client.request<DeleteWorkMutation>({ document: DeleteWorkDocument, variables, requestHeaders: { ...requestHeaders, ...wrappedRequestHeaders }, signal }), 'DeleteWork', 'mutation', variables);
    }
  };
}
