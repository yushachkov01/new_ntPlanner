...
export type FetchExecutorRolesQuery = {
  __typename?: 'query_root';
  executor: Array<{ __typename?: 'executor'; role: string }>;
};

export const FetchExecutorRolesDocument = gql`
  query FetchExecutorRoles {
    executor(distinct_on: role) {
      role
    }
  }
`;
...
export function getSdk(client: GraphQLClient, withWrapper: SdkFunctionWrapper = defaultWrapper) {
  return {
    FetchExecutorRoles(
      variables?: FetchExecutorRolesQueryVariables,
      requestHeaders?: GraphQLClientRequestHeaders,
      signal?: RequestInit['signal'],
    ): Promise<FetchExecutorRolesQuery> {
      return withWrapper(
        (wrappedRequestHeaders) =>
          client.request<FetchExecutorRolesQuery>({
            document: FetchExecutorRolesDocument,
            variables,
