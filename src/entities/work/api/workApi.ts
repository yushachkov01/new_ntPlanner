...
export async function insertWork(object: InsertWorkMutationVariables['object']): Promise<number> {
  ...
  const idRaw = insert_works_one.idInt;
  if (typeof idRaw !== 'string') {
    throw new Error(`Unexpected insert_works_one.idInt: ${idRaw}`);
  }
  const idNum = Number(idRaw);
  if (Number.isNaN(idNum)) {
    throw new Error(`Cannot convert inserted idInt to number: ${idRaw}`);
  }
  return idNum;
}
 ...
 * @param idInt — первичный ключ записи
...
export async function updateWork(
  ...
  const variables: UpdateWorkMutationVariables = { idInt, set };
  const { update_works_by_pk } = await graphqlClient.request<
    UpdateWorkMutation,
    UpdateWorkMutationVariables
  >(UpdateWorkDocument, variables);

  if (!update_works_by_pk) {
    throw new Error('Update failed');
  }

  const updatedId = update_works_by_pk.idInt;
  if (typeof updatedId !== 'number') {
    throw new Error(`Unexpected update_works_by_pk.idInt: ${updatedId}`);
  }
  return updatedId;
}
